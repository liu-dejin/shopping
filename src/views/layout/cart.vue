...
<div class="cart-box" v-if="isLogin && cartList.length > 0">
 <!-- 购物车开头 -->
    ...
    <!-- 购物车列表 -->
    <div class="cart-list">
      <div class="cart-item" v-for="item in cartList" :key="item.googs_id">
        <!-- 小选框 -->
    ...
    goPay () {
      ...
      if (this.selCount > 0) {
        ...
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(item => item.id).join(',') // cartId,cartId,cartId 购物车集
          }
        })
      }
    }
